...
do
    ...
    vcfgz=${i}
    filtered_vcfgz=${vcfgz%.vcf.gz}.permissive.vcf.gz
    if [ ! -f ${filtered_vcfgz} ]
        then
        out=../log/gatk/filter.${window_id}.permissive.out
        log=../log/gatk/filter.${window_id}.permissive.log
        # Remove log files if they exist already.
        rm -f ${out}
        rm -f ${log}
        sbatch -o ${out} apply_permissive_filter.slurm ${vcfgz} ../data/reference/orenil2.fasta ../data/masks/mapability_mask_100_90.bed ${filtered_vcfgz} ${log}
