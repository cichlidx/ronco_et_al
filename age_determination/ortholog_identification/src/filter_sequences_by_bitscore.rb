...
refgroup = ARGV[2].split(",")
relative_bitscore_threshold = ARGV[3].to_f
...
filenames_in.each do |f|
	...
	maximum_refgroup_bitscore = 0
	maximum_ingroup_bitscore = 0
	n_known_bases_in_best_refgroup_seq = 0
	n_known_bases_in_best_ingroup_seq = 0
	1.upto(fasta_bitscores.size-1) do |x|
		bitscore = fasta_bitscores[x]
		unless bitscore == "None"
			if refgroup.include?(fasta_ids[x])
				if maximum_refgroup_bitscore < bitscore.to_f
					maximum_refgroup_bitscore = bitscore.to_f
					n_known_bases_in_best_refgroup_seq = fasta_seqs[x].upcase.count("A") + fasta_seqs[x].upcase.count("C") + fasta_seqs[x].upcase.count("G") + fasta_seqs[x].upcase.count("T")
				end
			else
				if maximum_ingroup_bitscore < bitscore.to_f
					maximum_ingroup_bitscore = bitscore.to_f
					n_known_bases_in_best_ingroup_seq = fasta_seqs[x].upcase.count("A") + fasta_seqs[x].upcase.count("C") + fasta_seqs[x].upcase.count("G") + fasta_seqs[x].upcase.count("T")
				end
			end
		end
	end
	...
	fasta_ids.size.times do |x|
		...
		if fasta_bitscores[x] == "None"
			...
			if refgroup.include?(fasta_ids[x])
				if n_known_bases_in_this_seq < n_known_bases_in_best_refgroup_seq
					...
					correction = ((n_known_bases_in_this_seq/n_known_bases_in_best_refgroup_seq.to_f)+1.0)/2.0
				else
					correction = 1.0
				end
				if fasta_bitscores[x].to_f < correction * relative_bitscore_threshold * maximum_refgroup_bitscore
					fasta_seqs[0].size.times {new_fasta_string << "-"}
					new_fasta_string << "\n"
				...
				if n_known_bases_in_this_seq < n_known_bases_in_best_ingroup_seq
					correction = ((n_known_bases_in_this_seq/n_known_bases_in_best_ingroup_seq.to_f)+1.0)/2.0
				else
					correction = 1.0
				end
