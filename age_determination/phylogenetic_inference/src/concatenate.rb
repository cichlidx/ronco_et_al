# m_matschiner Sat Nov 24 13:50:12 CET 2018
...
help_string << "  -p       none                   | Write a partitions block (only for nexus and phylip format)\n"
help_string << "\n"
...
# Check whether a partitions block should be written.
write_partitions_block = false
if ARGV.include?("-p")
    write_partitions_block = true
    ARGV[ARGV.index("-p")] = nil
    ARGV.compact!
end
...
input_file_names.each do |a|
    ...
    seq_lengths_per_file << file_seqs[0].size
...
if format == "phylip"
    ...
    if write_partitions_block
        out_string << "\n"
        cumulative_length = 0
        input_file_names.size.times do |x|
            out_string << "DNA, #{input_file_names[x].split("/").last.chomp(".nex").chomp(".fasta").chomp(".phy")}=#{cumulative_length+1}-#{cumulative_length+seq_lengths_per_file[x]}\n"
            cumulative_length += seq_lengths_per_file[x]
        end
    end
    ...
    if write_partitions_block
        out_string << "\n"
        out_string << "begin sets;\n"
        out_string << "  charpartition allparts =\n"
        cumulative_length = 0
        input_file_names.size.times do |x|
            if x == input_file_names.size-1
                out_string << "  #{input_file_names[x].split("/").last.chomp(".nex").chomp(".fasta").chomp(".phy")}: #{cumulative_length+1}-#{cumulative_length+seq_lengths_per_file[x]};\n"
            else
                out_string << "  #{input_file_names[x].split("/").last.chomp(".nex").chomp(".fasta").chomp(".phy")}: #{cumulative_length+1}-#{cumulative_length+seq_lengths_per_file[x]},\n"
            end
            cumulative_length += seq_lengths_per_file[x]
    ...
    unique_ids.size.times do |x|
        ...
        out_string << "#{concatenated_seqs[x]}\n"
...
concatenated_seqs.size.times do |x|
    if concatenated_seqs[x].gsub("-","").length == 0
        puts "WARNING: No sequence information available for #{unique_ids[x]}!"
        puts concatenated_seqs[x]
        exit
    end
end
