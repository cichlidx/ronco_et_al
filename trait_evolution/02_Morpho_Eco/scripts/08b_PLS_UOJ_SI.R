...
SI= sp_means_SI[sp_means_SI$sp %in%  dimnames(sp_means_OJ)[[3]],]

###  keep only morpho data which is in SI data
body= sp_means_OJ[,, dimnames(sp_means_OJ)[[3]] %in% SI$sp  ]
...
print(summary(pgls(YScores ~ XScores , compset, lambda= 'ML', bounds=list(lambda=c(0,1.03)))))


################################################
######  save PLS scores to file
#head(f1)
write.csv( f1, "../oral/PLS_oral_SI.txt", quote=F, row.names=F)
...
## recenter shapes
maxshape[,1] = maxshape[,1]+abs(mean(maxshape[,1]))
minshape[,1] = minshape[,1]+abs(mean(minshape[,1]))
...
quartz(file="../Plots/UOJ_SI_PLS.pdf", type="pdf", width=7, height=7)
par(mar = c(5,12,2,2))

plot( f1$ XScores, f1$ YScores, col=f1$tribe, pch=16, xlab= "stabel isotopes projection", ylab="Oral jaw shape projection", las=1)
...
screen(1)
par(mar = c(0,0,0,0))
plot(maxshape, xlim=c(-0.1,0.1), ylim=c(-0.1,0.1), pch=16, cex=0.3, axes=F, xlab="", ylab="")
lines(maxshape[c(3,1,4,2),])

screen(13)
par(mar = c(0,0,0,0))
plot(minshape, xlim=c(-0.1,0.1), ylim=c(-0.1,0.1), pch=16, cex=0.3, axes=F, xlab="", ylab="")
lines(minshape[c(3,1,4,2),])
