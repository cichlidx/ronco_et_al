# m_matschiner Wed Jan 9 18:22:00 CET 2019
...
if format == "phylip"
    ...
    if write_partitions_block
        ...
        cumulative_length = 0
        input_file_names.size.times do |x|
            out_string << "  charset #{input_file_names[x].split("/").last.chomp(".nex").chomp(".fasta").chomp(".phy")} = #{cumulative_length+1}-#{cumulative_length+seq_lengths_per_file[x]};\n"
            cumulative_length += seq_lengths_per_file[x]
        end
        out_string << "end;\n"
    end
