...
# Feedback.
puts "Asymmetry table: #{asymmetry_table_file_name}"
puts "Heatmap name order: #{heatmap_name_order_file_name}"
puts "Maximum D to plot: #{max_d_value}"
puts "Plot: #{plot_file_name}"
STDOUT.flush
...
asymmetry_table_lines.each do |l|
	...
	p2s << asymmetry_table_line_ary[1]
	p3s << asymmetry_table_line_ary[2]
	d_values << asymmetry_table_line_ary[6].to_f
	p_values << asymmetry_table_line_ary[7].to_f
end

# Prepare matrices for d and p.
d_matrix = []
p_matrix = []
...
names_ordered.size.times do |x|
	(x+1).upto(names_ordered.size-1) do |y|
		selected_d_value = 0.0
		selected_p_value = 1.0
		print "Analyzing pairs with species #{x+1} out of #{names_ordered.size}..."
		STDOUT.flush
		p2s.size.times do |z|
			if [p2s[z],p3s[z]] == [names_ordered[x],names_ordered[y]] or [p3s[z],p2s[z]] == [names_ordered[x],names_ordered[y]]
				if p_values[z] < selected_p_value
					selected_d_value = d_values[z]
					selected_p_value = p_values[z]
				elsif p_values[z] == selected_p_value and d_values[z] > selected_d_value
					selected_d_value = d_values[z]
				end
			end
		end
...
# Feedback.
puts "Write file #{plot_file_name}."