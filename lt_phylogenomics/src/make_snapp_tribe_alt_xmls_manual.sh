...
## Fixed input.
# Set the vcf file.
gzvcf=../data/vcf/permissive.phased.vcf.bgzip.gz
...
for analysis_id in tropheini1 tropheini2 # Previously made (no need to make again): trematocarini lamprologini1 lamprologini5 orthochromini serranochromini astatotilapini
do
	...
	# Set the name of the original snapp xml file (astral version).
	...
	if (( ${n_sites} < 1000 ))
	then
		# Set the names of the alternative snapp xml files based on the permissive dataset.
		alt_xml_astral=../res/snapp/${analysis_id}/xml/${analysis_id}_astral_topology_alt.xml
		alt_xml_raxml=../res/snapp/${analysis_id}/xml/${analysis_id}_raxml_topology_alt.xml
		alt_xml_free=../res/snapp/${analysis_id}/xml/${analysis_id}_free_topology_alt.xml
		...
		ruby snapp_prep.rb -v tmp.vcf -t tmp.samples.txt -c ${constraint_astral} -m 10000 -l 250000 -w 0 -s ${starting_astral} -o ${analysis_id}_astral_topology -x ${alt_xml_astral}
		ruby snapp_prep.rb -v tmp.vcf -t tmp.samples.txt -c ${constraint_raxml} -m 10000 -l 250000 -w 0 -s ${starting_raxml} -o ${analysis_id}_raxml_topology -x ${alt_xml_raxml}
		ruby snapp_prep.rb -v tmp.vcf -t tmp.samples.txt -c ${constraint_free} -m 10000 -l 250000 -s ${starting_raxml} -o ${analysis_id}_free_topology -x ${alt_xml_free}
